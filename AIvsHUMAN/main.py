    ...
    def doMove(self, player_pits, pit):
        ...
        while seeds > 0:
            ...
            if isinstance(current_pit, int) and (
                (player_pits == self.player1_pits and current_pit == 2)
                or (player_pits == self.player2_pits and current_pit == 1)
            ):
                continue
...
class MancalaPygame:
    ...
    def start_screen(self):
        ...
        while True:
            ...
            you_button = self.draw_button(
                ...
                200,
                75,
                pygame.Rect(self.WIDTH // 2 - 200, 300, 200, 75).collidepoint(
                    mouse_pos
                ),
            )
            ...
            instruction = self.text_font.render(
                "Click to select who goes first", True, TEXT_COLOR
            )
    ...
    def draw_board(self):
        ...
        for i in range(self.HEIGHT):
            r = int(247 * (1 - i / self.HEIGHT) + 200 * (i / self.HEIGHT))
            g = int(236 * (1 - i / self.HEIGHT) + 180 * (i / self.HEIGHT))
            b = int(216 * (1 - i / self.HEIGHT) + 150 * (i / self.HEIGHT))
            pygame.draw.line(self.screen, (r, g, b), (0, i), (self.WIDTH, i))
        # Game board with shadow and rounded corners
        board_rect = pygame.Rect(200, 50, 980, 600)
        board_shadow = board_rect.inflate(15, 15)
        pygame.draw.rect(self.screen, (100, 100, 100), board_shadow, border_radius=50)
        pygame.draw.rect(self.screen, BOARD_COLOR, board_rect, border_radius=40)

        # Draw pits and stores
        for pit, (x, y, width, height) in self.pit_positions.items():
            color = PIT_COLOR

            # Draw pits and stores with a glassy effect
            if isinstance(pit, int):  # Stores
                pygame.draw.rect(
                    self.screen,
                    (100, 100, 100),  # Light gray for glassy effect
            ...
            self.draw_elegant_seeds(x + 5, y + 5, width - 10, height - 10, seed_count)

            # Seed count text with a modern style
            seed_text = self.text_font.render(str(seed_count), True, (50, 50, 50))
            text_rect = seed_text.get_rect(center=(x + width // 2, y + height + 20))

            # Add a subtle shadow for the text
            shadow_text = self.text_font.render(str(seed_count), True, (150, 150, 150))
            shadow_rect = shadow_text.get_rect(
                center=(x + width // 2 + 2, y + height + 22)
            )
            self.screen.blit(shadow_text, shadow_rect)
            self.screen.blit(seed_text, text_rect)

        # Title with gradient and shadow
        title = self.title_font.render("Mancala", True, (255, 255, 255))
        shadow_title = self.title_font.render("Mancala", True, (100, 100, 100))

        self.screen.blit(
            shadow_title, (self.WIDTH // 2 - title.get_width() // 2 + 3, 13)
        )
        self.screen.blit(title, (self.WIDTH // 2 - title.get_width() // 2, 10))

        # Player turn indicator with dynamic styling
        player_text = "Your Turn" if self.current_player == -1 else "Computer's Turn"
        player_color = (50, 200, 50) if self.current_player == -1 else (200, 50, 50)
        player_render = self.subtitle_font.render(player_text, True, player_color)

        # Add glow effect for turn indicator
        glow_render = self.subtitle_font.render(player_text, True, (255, 255, 150))
        self.screen.blit(
            glow_render,
            (self.WIDTH // 2 - glow_render.get_width() // 2, 30),
        )
        self.screen.blit(
            player_render,
            (self.WIDTH // 2 - player_render.get_width() // 2, 25),
        )

        # Add clickable text for instructions
        instruction_text = "Click on your pits to make a move!"
        instruction_render = self.text_font.render(instruction_text, True, (0, 0, 0))
        self.screen.blit(
            instruction_render,
            (self.WIDTH // 2 - instruction_render.get_width() // 2, self.HEIGHT - 40),
        )
    ...
    def check_game_over(self):
        ...
        player1_empty = all(
            self.board.board[pit] == 0 for pit in self.board.player1_pits
        )
        player2_empty = all(
            self.board.board[pit] == 0 for pit in self.board.player2_pits
        )

        if player1_empty or player2_empty:
            # Collect remaining seeds
            remaining_pits = (
                self.board.player2_pits if player1_empty else self.board.player1_pits
            )
    ...
    def run(self):
        ...
        winner_text = (
            "You Win!"
            if self.board.board[2] < self.board.board[1]
            else "Computer Wins!"
        )
        final_score = (
            f"Final Score: You {self.board.board[1]} - {self.board.board[2]} Computer"
        )
        ...
        self.screen.blit(
            winner, (self.WIDTH // 2 - winner.get_width() // 2, self.HEIGHT // 2 - 100)
        )
        self.screen.blit(
            score, (self.WIDTH // 2 - score.get_width() // 2, self.HEIGHT // 2)
        )
        self.screen.blit(
            restart,
            (self.WIDTH // 2 - restart.get_width() // 2, self.HEIGHT // 2 + 100),
        )
