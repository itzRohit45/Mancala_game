...
class MancalaBoard:
    ...
    def doMove(self, player_pits, pit):
        ...
        return isinstance(current_pit, int) and current_pit == (
            1 if player_pits == self.player1_pits else 2
        )
...
class Game:
    def __init__(self, human_side="G", computer_side="A"):
        ...
        self.playerSide = {1: computer_side, -1: human_side}

    def gameOver(self):
        # Check if either player has no seeds in their pits
        player1_empty = all(
            self.state.board[pit] == 0 for pit in self.state.player1_pits
        )
        player2_empty = all(
            self.state.board[pit] == 0 for pit in self.state.player2_pits
        )

        if player1_empty or player2_empty:
            # Collect remaining seeds
            remaining_pits = (
                self.state.player2_pits if player1_empty else self.state.player1_pits
            )
    ...
    def evaluate_ai2(self):
        ...
        player1_total_seeds = sum(
            self.state.board[pit] for pit in self.state.player1_pits
        )
        player2_total_seeds = sum(
            self.state.board[pit] for pit in self.state.player2_pits
        )
...
class Play:
    ...
    def MinimaxAlphaBetaPruning(self, game, player, depth, alpha, beta, ai_type="ai1"):
        if game.gameOver() or depth == 0:
            evaluation_func = (
                game.evaluate_ai1 if ai_type == "ai1" else game.evaluate_ai2
            )
    ...
    def ai1_turn(self):
        ...
        self.game_ai1.state.doMove(
            self.game_ai1.state.player1_pits
            if best_move in self.game_ai1.state.player1_pits
    ...
    def ai2_turn(self):
        ...
        self.game_ai2.state.doMove(
            self.game_ai2.state.player1_pits
            if best_move in self.game_ai2.state.player1_pits
    ...
    def play_ai_vs_ai(self):
        ...
        while not self.game_ai1.gameOver():
            ...
            if ai_turn == 1:
                ...
                extra_turn = any(
                    self.game_ai1.state.board[pit] == 0
                    for pit in self.game_ai1.state.player1_pits
                )
                ...
                extra_turn = any(
                    self.game_ai2.state.board[pit] == 0
                    for pit in self.game_ai2.state.player2_pits
                )
                ai_turn = -1 if extra_turn else 1
